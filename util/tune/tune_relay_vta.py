...
from Fake_mobilenetG import mx_MobileNet, load_mobilenet
import mxnet
...
def compile_network(env, target, model, start_pack, stop_pack):
    ...
    input_name = "input0"
    dtype = "int8"
    shape_list = [(input_name, [env.BATCH,3,224,224])]

    #my_model = MobileNetV1(ch_in=3,n_classes=1000)
    #my_model.load_state_dict(torch.load("/home/harry09119/tvm/utils/model/torch_mobilenet"))
    #my_model.eval()

    #input_shape = [1, 3, 224, 224]
    #input_data = torch.randn(input_shape)
    #my_model = torch.jit.trace(my_model, input_data).eval()

    # Get off the shelf gluon model, and convert to relay
    my_model = mxnet.gluon.model_zoo.vision.get_model(model, pretrained=True)
...
tracker_port = int(os.environ.get("TVM_TRACKER_PORT", 8103))
