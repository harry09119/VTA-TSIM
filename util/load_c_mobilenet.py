...
def profile():
    ...
    print("Extract tasks...")
    relay_prog, params, net = compile_network(env, target, network, start_pack, stop_pack)
    ...
    with autotvm.tophub.context(target):
        ...
        # Export library
        print("Upload...")
        lib.export_library("compiled_mobilenet.tar")
        """
        temp = utils.tempdir()
        ...
        #net.forward(Batch([mx.nd.array(image)]))
        #prob = net.get_outputs()[0].asnumpy()
        #prob = np.squeeze(prob)

        #net_outs = np.argsort(prob)[::-1]
        print("TVM: ",m_out)
        #print("NET: ",net_out)
        #tvm_out = m.debug_get_output(0, tvm.nd.empty((1,1000))).numpy()
